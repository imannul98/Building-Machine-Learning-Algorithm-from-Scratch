...
class LogisticRegression(object):
    ...
    def gradient(self, x_aug: np.ndarray, y: np.ndarray, h_x: np.ndarray
		...
		"""
        return np.dot(x_aug.T, (h_x - y)) / x_aug.shape[0]
    ...
    def accuracy(self, y: np.ndarray, y_hat: np.ndarray) ->float:
		...
		"""
        false=np.where(y-y_hat==0,1,0)
        return (np.sum(false))/len(y)
    ...
    def evaluate(self, x: np.ndarray, y: np.ndarray, theta: np.ndarray
		...
		"""
        x=self.bias_augment(x)
        h_x=self.predict_probs(x,theta)
        y_hat=self.predict_labels(h_x)
        return self.loss(y,h_x),self.accuracy(y,y_hat)
    ...
    def fit(self, x_train: np.ndarray, y_train: np.ndarray, x_val: np.
        ...
        x_augment=self.bias_augment(x_train)
        for x in range(epochs):
            h_x=self.predict_probs(x_augment,theta)
            gradient=self.gradient(x_augment,y_train,h_x)
            theta -= lr * gradient
            if x % 100 == 0 or x == 0:
                self.update_evaluation_lists(x_train,y_train,x_val,y_val,theta,x)
        return theta
